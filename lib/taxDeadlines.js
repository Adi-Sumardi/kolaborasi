...
/**
 * Get billing sent deadline (tanggal 13 bulan berikutnya)
 * Billing/invoice harus dikirimkan ke klien paling lambat tgl 13 bulan berikutnya
 */
export function getBillingDeadline(month, year) {
  const nextMonth = month === 12 ? 1 : month + 1;
  const nextYear = month === 12 ? year + 1 : year;
...
/**
 * Get payment follow-up deadline (tanggal 20 bulan berikutnya)
 * Tenggat klien melunasi tagihan
 */
export function getPaymentDueDeadline(month, year) {
  const nextMonth = month === 12 ? 1 : month + 1;
  const nextYear = month === 12 ? year + 1 : year;
  return new Date(nextYear, nextMonth - 1, 20);
}
