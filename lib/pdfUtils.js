...
export function generateAllKPIPDF({ kpiList, divisions, dateRange, divisionFilter }) {
  const doc = new jsPDF();

  // Get division name for filter
  const getDivisionName = (divisionId) => {
    const division = divisions.find(d => d.id === divisionId);
    return division?.name || '-';
  };

  const filterDivisionName = divisionFilter === 'all'
    ? 'Semua Divisi'
    : getDivisionName(divisionFilter);

  // Header
  doc.setFontSize(20);
  doc.setTextColor(37, 99, 235);
  doc.text('LAPORAN KPI SEMUA KARYAWAN', 105, 20, { align: 'center' });
  ...
  doc.setFontSize(10);
  doc.setTextColor(0, 0, 0);
  doc.text(`Periode: ${new Date(dateRange.startDate).toLocaleDateString('id-ID')} - ${new Date(dateRange.endDate).toLocaleDateString('id-ID')}`, 20, 35);
  doc.text(`Filter Divisi: ${filterDivisionName}`, 20, 41);
  doc.text(`Total Karyawan: ${kpiList.length}`, 20, 47);
  doc.text(`Tanggal Cetak: ${new Date().toLocaleDateString('id-ID')}`, 20, 53);
  ...
  doc.setFontSize(12);
  doc.setTextColor(37, 99, 235);
  doc.text('RINGKASAN', 105, 70, { align: 'center' });

  const avgScore = kpiList.length > 0
    ? (kpiList.reduce((sum, k) => sum + k.score, 0) / kpiList.length).toFixed(1)
    : 0;
  const avgCompletion = kpiList.length > 0
    ? Math.round(kpiList.reduce((sum, k) => sum + k.completionRate, 0) / kpiList.length)
    : 0;

  doc.setFontSize(10);
  doc.setTextColor(0, 0, 0);
  doc.text(`Rata-rata KPI Score: ${avgScore}`, 30, 78);
  doc.text(`Rata-rata Completion: ${avgCompletion}%`, 110, 78);

  // KPI Table
  const tableData = kpiList.map((kpi, index) => [
    (index + 1).toString(),
    kpi.userName,
    getDivisionName(kpi.divisionId),
    kpi.score.toFixed(1),
    `${kpi.completionRate}%`,
    `${kpi.completedJobdesks}/${kpi.totalJobdesks}`,
    kpi.totalHours.toString(),
    kpi.totalLogs.toString()
  ]);

  autoTable(doc, {
    startY: 92,
    head: [['#', 'Nama', 'Divisi', 'Score', 'Rate', 'Jobdesk', 'Jam', 'Log']],
    body: tableData,
    theme: 'grid',
    headStyles: {
      fillColor: [37, 99, 235],
  ...
  // Save PDF
  const fileName = `KPI_Report_All_Employees_${new Date().getTime()}.pdf`;
  doc.save(fileName);
}
