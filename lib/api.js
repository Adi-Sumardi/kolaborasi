...
// Notify Electron main process about auth changes
export const notifyElectronAuth = (token, user) => {
  if (typeof window !== 'undefined' && window.electronAPI?.isElectron && token && user?.id) {
    window.electronAPI.notifyLogin(token, user);
  }
};
...
export const removeToken = () => {
  if (typeof window !== 'undefined') {
    ...
    // Notify Electron to stop agent on logout
...
export const jobdeskAPI = {
  getAll: (params = {}) => {
    ...
    return apiRequest(`jobdesks${qs ? `?${qs}` : ''}`);
  },
  ...
  bulkCreate: (jobdesks) => apiRequest('jobdesks/bulk-create', {
    method: 'POST',
    body: JSON.stringify({ jobdesks }),
  }),
  ...
  updateAdjustment: (id, data) => apiRequest(`jobdesks/${id}/adjustment`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),
  ...
  // Comments
  getComments: (jobdeskId) => apiRequest(`jobdesks/${jobdeskId}/comments`),

  createComment: (jobdeskId, comment, taskType) => apiRequest(`jobdesks/${jobdeskId}/comments`, {
    method: 'POST',
    body: JSON.stringify({ comment, taskType: taskType || null }),
  }),

  updateComment: (jobdeskId, commentId, comment) => apiRequest(`jobdesks/${jobdeskId}/comments/${commentId}`, {
    method: 'PUT',
    body: JSON.stringify({ comment }),
  }),

  deleteComment: (jobdeskId, commentId) => apiRequest(`jobdesks/${jobdeskId}/comments/${commentId}`, {
    method: 'DELETE',
  }),
};
...
export const rekapAPI = {
  getKaryawan: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`rekap/karyawan${queryString ? `?${queryString}` : ''}`);
  },
  getKaryawanDetail: (userId, params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`rekap/karyawan/${userId}${queryString ? `?${queryString}` : ''}`);
  },
};
...
export const profileAPI = {
  ...
  getAttachments: (userId, params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`profile/${userId}/attachments${queryString ? `?${queryString}` : ''}`);
  },
...
export const userAPI = {
  ...
  changeOwnPassword: (id, currentPassword, newPassword) => apiRequest(`users/${id}/password`, {
    method: 'PUT',
    body: JSON.stringify({ currentPassword, newPassword }),
  }),

  updateOwnEmail: (id, currentPassword, newEmail) => apiRequest(`users/${id}/email`, {
    method: 'PUT',
    body: JSON.stringify({ currentPassword, newEmail }),
  }),

  updateOwnProfile: (id, name) => apiRequest(`users/${id}`, {
    method: 'PUT',
...
export const sp2dkAPI = {
  ...
  confirm: (id, data) => apiRequest(`sp2dk/${id}/confirm`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),
...
// Monitor API (Screen Monitoring)
export const monitorAPI = {
  getSessions: () => apiRequest('monitor/sessions'),
  connect: (monitorCode) => apiRequest('monitor/connect', {
    method: 'POST',
    body: JSON.stringify({ monitorCode }),
  }),
};
...
export const billingAPI = {
  ...
  create: (data) => apiRequest('billing', {
    method: 'POST',
    body: JSON.stringify(data),
  }),
  update: (id, data) => apiRequest(`billing/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),
  delete: (id) => apiRequest(`billing/${id}`, { method: 'DELETE' }),
  uploadFile: (id, formData) => apiRequest(`billing/${id}/upload`, {
    method: 'POST',
    body: formData,
    headers: {}, // biarkan browser set Content-Type multipart boundary
  }),
};

// Staff Output Monitoring API
export const monitoringAPI = {
  getStaffOutput: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`monitoring/staff-output${queryString ? `?${queryString}` : ''}`);
  },
};

// Work Session API (Jam Kerja)
export const workSessionAPI = {
  clockIn: (data = {}) => apiRequest('work-sessions/clock-in', {
    method: 'POST',
    body: JSON.stringify(data),
  }),
  clockOut: () => apiRequest('work-sessions/clock-out', {
    method: 'PUT',
  }),
  getSessions: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`work-sessions${queryString ? `?${queryString}` : ''}`);
  },
  getSummary: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`work-sessions/summary${queryString ? `?${queryString}` : ''}`);
  },
};
