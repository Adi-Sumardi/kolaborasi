...
export const generateKPIPDF = ({ user, kpiData, logs, dateRange, barData }) => {
  const doc = new jsPDF();
  ...
  let barColor;
  if (scorePercentage >= 80) barColor = [34, 197, 94]; // Green
...
export const generateKPIReport = (kpiData, userData, dailyLogs, dateRange) => {
  const doc = new jsPDF();
  ...
  doc.text(`Role: ${getRoleLabel(userData.role)}`, 20, 54);
  if (userData.division) {
    doc.text(`Divisi: ${userData.division.name}`, 20, 60);
  }
  
  // Period
  doc.text(`Periode: ${new Date(dateRange.startDate).toLocaleDateString('id-ID')} - ${new Date(dateRange.endDate).toLocaleDateString('id-ID')}`, 20, 66);
  doc.text(`Tanggal Cetak: ${new Date().toLocaleDateString('id-ID')}`, 20, 72);
  ...
  doc.setFontSize(14);
  doc.setTextColor(37, 99, 235);
  doc.text('RINGKASAN KPI', 105, 90, { align: 'center' });
  
  doc.setFontSize(11);
  doc.setTextColor(0, 0, 0);
  const summaryY = 98;
  doc.text(`KPI Score: ${kpiData.score || 0} / 100`, 30, summaryY);
  doc.text(`Tingkat Penyelesaian: ${kpiData.completionRate || 0}%`, 30, summaryY + 7);
  doc.text(`Jobdesk Selesai: ${kpiData.completedJobdesks || 0} / ${kpiData.totalJobdesks || 0}`, 110, summaryY);
  doc.text(`Total Jam Kerja: ${kpiData.totalHours || 0} jam`, 110, summaryY + 7);
  doc.text(`Total Log Aktivitas: ${kpiData.totalLogs || 0} entri`, 110, summaryY + 14);
  ...
  if (dailyLogs && dailyLogs.length > 0) {
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text('Log Aktivitas Harian:', 20, 145);
    
    const tableData = dailyLogs.slice(0, 15).map(log => [
      new Date(log.date).toLocaleDateString('id-ID'),
      log.notes.substring(0, 50) + (log.notes.length > 50 ? '...' : ''),
      `${log.hoursSpent || 0} jam`
    ]);
  ...
  // Save PDF
  const fileName = `KPI_Report_${userData.name.replace(/\s+/g, '_')}_${new Date().getTime()}.pdf`;
  doc.save(fileName);
};
